...
do
	...
	INI=`cat /home/diogo/MEGA/cenas/SAT-assembly/Results/scripts/files/solution_ico | sed -n 1','1'p' | awk '{print $1}'`
	if [ $INI = "UNSAT" ];then
		echo "No solution for icosahedron!"
		exit 0
	fi
	...
	rm -f '/home/diogo/MEGA/cenas/SAT-assembly/Results/scripts/files/test.snubdode.fixed.Na'$SPECIES'Nc'$COLORS'.cls'
        python3 generate_cls_file.snubcube.py snubdode "$SPECIES" "$COLORS" '/home/diogo/MEGA/cenas/SAT-assembly/Results/scripts/files/test' 1 > dump
        sed -n '2p' files/solution_ico | sed 's/'$TOT'.*//' | sed 's/$/'$TOT'/' | sed 's/ /a /g' | tr ' ' '\n' | sed 's/a/ 0/g' | sed 's/'$TOT'/'$TOT' 0/g' >> '/home/diogo/MEGA/cenas/SAT-assembly/Results/scripts/files/test.snubdode.fixed.Na'$SPECIES'Nc'$COLORS'.cls'
        ...
        RESS=`cat /home/diogo/MEGA/cenas/SAT-assembly/Results/scripts/files/solution_dode | sed -n 1','1'p' | awk '{print $1}'`
	...
	if [ $RES = "SAT" ] || [ $RESS = "SAT" ] || [ $RETS = "SAT" ] || [ $RETTS = "SAT" ]; then
		...
		sed -n '2p' files/solution_ico | sed 's/'$TOT'.*//' | sed 's/$/'$TOT'/' | sed -e 's/^/ /' | sed 's/ / -/g' | sed 's/ --/ /g' | sed 's/^[ \t]*//' | sed 's/'$TOT'/'$TOT' 0/g' >> '/home/diogo/MEGA/cenas/SAT-assembly/Results/scripts/files/test.icosahedron.fixed.Na'$SPECIES'Nc'$COLORS'.cls'
		...
		i=$END
		python3 generate_cls_file.icosahedron.py icosahedron "$SPECIES" "$COLORS" '/home/diogo/MEGA/cenas/SAT-assembly/Results/scripts/files/solution_ico' 0
